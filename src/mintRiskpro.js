...
const main = async () => {
  ...
  if (appMode === 'MoC') {
    // Collateral Coinbase
    const { receipt, filteredEvents } = await mintRiskpro(web3, dContracts, config, amountRiskpro)
  } else {
    // Collateral RRC20
    const { receipt, filteredEvents } = await mintRiskproRRC20(web3, dContracts, config, amountRiskpro)
  }
}
